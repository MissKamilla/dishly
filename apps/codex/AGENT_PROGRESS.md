...
- Step 25: добавлены шесть минимальных синтетических HTML fixtures рядом с parser tests: direct Recipe, Recipe внутри `@graph`, массив JSON-LD, `HowToSection`, документ без Recipe и malformed JSON-LD. Fixtures не содержат analytics/tracking и не зависят от сети; отдельный тест читает файлы и подтверждает каждый сценарий. Проверка: полный backend suite — 22 suites, 259 tests passed; build и lint прошли.
- Step 26: проведен аудит обязательной unit-test матрицы. Уже покрыты supported/unsupported/localhost/protocol/invalid URL и unsafe redirect; success/timeout/non-HTML/HTTP/oversize fetch; direct/array/@graph/multiple/malformed/missing JSON-LD; все normalizers, groups и orchestration с mocked HTTP. Добавлен недостающий end-to-end unit case минимально валидного Recipe, подтверждающий null для всех отсутствующих optional fields. PostgreSQL, Redis и live Good Food в unit tests не используются. Проверка: полный backend suite — 22 suites, 260 tests passed; build и lint прошли.
- Step 27: выполнена отдельная live-проверка настоящим `RecipeParserService` для `https://www.bbcgoodfood.com/recipes/marry-me-chicken`. Первая sandbox-попытка корректно вернула retryable DNS failure; повтор с разрешенным network access успешен. Получено: title=`'Marry me' chicken`, непустое description, servings=4, prepTimeMinutes=20, cookTimeMinutes=45, 12 ingredients, 4 steps. Значения извлечены parser-ом из live HTML, не захардкожены. Код и unit tests на этом шаге не менялись.
- Step 28: добавлен dev-only `src/scripts/parser-smoke.ts` и npm-команда `parser:smoke`. Запуск: `npm run parser:smoke -- <Good Food URL>`. Скрипт напрямую использует настоящий `RecipeParserService`, не поднимает Nest context/PostgreSQL/Redis/queue и выводит только title, servings, ingredient count, step count. Live smoke для marry-me-chicken успешно вывел 4 servings, 12 ingredients, 4 steps; sandbox DNS failure перед сетевым повтором корректно сообщил retryable error. Проверка: полный backend suite — 22 suites, 260 tests passed; build и lint прошли.
- Step 29: подтвержден пустой staged/unstaged diff для `recipe-import.queue.ts`, `recipe-import.processor.ts`, `recipe-import.contract.ts`; parser не вызывается worker-ом, payload и attempts/backoff не изменены. Queue unit tests: 2 suites, 13 tests passed. `npm run queue:smoke` был запущен, но live-проверка не завершилась из-за недоступных PostgreSQL `127.0.0.1:5433` и Redis `127.0.0.1:6380`; Docker отсутствует в текущем WSL. Конфигурация ради окружения не изменялась, поэтому инфраструктурный smoke остается непроверенным.
- Step 30: подтвержден пустой staged/unstaged diff для Recipe/RecipeIngredient/RecipeStep entities, RecipeStatus и database migrations. Parser не импортирует Repository/TypeORM и не вызывает persistence operations; parser-specific database fields и migrations не добавлены. Связанные mapper/service/status DTO regression tests: 3 suites, 15 tests passed; backend build прошел. Migration commands не запускались, потому что schema не менялась и PostgreSQL в окружении недоступен.
- Step 31: подтвержден пустой staged/unstaged diff для `apps/frontend`; frontend package manifests и корневые dependency manifests не менялись. Add Recipe modal, recipe cards/details, processing UI и TanStack Query hooks не добавлялись; React dependencies не устанавливались. На этом шаге код не менялся.
- Step 32: подтверждено отсутствие OpenAI/Gemini/Ollama/Hugging Face/LangChain dependencies, imports, API keys и model configuration в parser scope, backend env example и manifests. Backend package-lock и env example не менялись. Parser остается полностью deterministic: allowlisted Good Food URL → HTML → JSON-LD → normalization. На этом шаге код не менялся.
- Step 33: аудит ограничений MVP подтверждает: поддерживаются только `bbcgoodfood.com`/`www.bbcgoodfood.com`; нет social parsers, browser automation/CAPTCHA bypass, AI extraction, shopping lists, nutrition calculations, translation, image persistence или parser registry. Совпадение `nutrition` есть только в тесте, подтверждающем игнорирование лишнего Schema.org поля. Ingredient normalizer лишь приводит известные названия единиц к сокращению и не выполняет quantity/unit conversion. Dependency changes отсутствуют. На этом шаге код не менялся.
- Step 34: финальные проверки Этапа 6 завершены. Из `apps/backend` успешно прошли `npm run build`, `npm run lint -- --no-fix`, полный `npm test -- --runInBand` (22 suites, 260 tests) и отдельная Auth/Recipes API/Queue regression (6 suites, 36 tests). `git diff HEAD --check` чистый; frontend, Auth, entities, RecipeStatus, migrations и BullMQ producer/processor/contract не изменены. `migration:show` не запускался из-за недоступного PostgreSQL; queue smoke также инфраструктурно заблокирован выключенными PostgreSQL/Redis и отсутствующим Docker, что явно зафиксировано в Step 29. Live parser и parser smoke успешно проверены. MUST FIX отсутствуют; реализация Этапа 6 готова к переходу на Этап 7 после отдельного разрешения разработчика.
- Исправления после независимого senior review: названия `HowToSection` и структурированное `ParsedIngredient.name` теперь проходят через общий `normalizePlainText`, поэтому внешняя HTML-разметка и содержимое script/style/template не попадают в эти display fields. Оригинальный ingredient `rawText` сохраняется без изменений. Добавлены security regression tests; итоговый backend suite — 22 suites, 262 tests passed; build и focused lint прошли.
- Текущий результат: `parse(url) → ParsedRecipe`. К Этапу 7 не переходить без отдельной команды.
